...
sut_ip=172.16.128.134
master_dns=172.16.128.133
domain=green.local
...
@test 'De server zou moeten ingesteld zijn als slave.' {
...
@test 'De server zou requests moeten forwarden naar de master server.' {
  result=$(grep 'masters' /etc/named.conf | fgrep "${master_dns}" )
  [ -n "${result}" ]
}

@test 'Er zou geen forward zone bestand mogen zijn.' {
